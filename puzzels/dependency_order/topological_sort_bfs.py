...
def find_dependency_order_bfs(graph):
    ...
    while len(start_nodes):
        ...
        que = []
        que.append(popped_node)
        while len(que):
            cur_node = que.pop(0)
            if cur_node not in visited:
                stack.append(cur_node)
            visited.add(cur_node)
            if cur_node in graph.outgoings:
                neighbors = graph.outgoings[cur_node]
...
graph.add_edge('B', 'D')
graph.add_edge('C', 'D')
res = find_dependency_order_bfs(graph)
print(res)
