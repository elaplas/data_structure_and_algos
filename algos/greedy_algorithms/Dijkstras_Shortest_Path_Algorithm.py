...
def dijkstra(graph, source):
    ...
    # None of the nodes are visited yet
    unvisited = set([node for node in graph.nodes])
    unvisited.remove(source)
    # By default the distance from each node to the source is infinite
    result = dict.fromkeys(unvisited, float('inf'))
    # The only known smallest distance to the source 
    result[source] = 0
    # By default the previous node to the source is the source itself
    path = dict.fromkeys(unvisited, source)
    ...
    while unvisited: 
        ...
        for unvisited_node in unvisited:
            ...
            cur_neighbours = graph.neighbours[unvisited_node]
            
            for neighbour in cur_neighbours:
                cur_dis = 0
                pair = (unvisited_node, neighbour)
                cur_dis += graph.distances[pair]
                cur_dis += result[neighbour]
                if cur_dis < result[unvisited_node]:
                    result[unvisited_node] = cur_dis
                    path[unvisited_node] = neighbour
            
            if min_node is None:
                min_node = unvisited_node
            elif result[unvisited_node] < result[min_node]:
                min_node = unvisited_node
            else:
                pass
                
        # The the node with the smallest distance in the current "result" list is 
        # removed from the "unvisited" list because there will not be any possible 
        # smaller paths from this node to the source since the path distance is caluclated either
        # by summing up the distance of corresponding neibour to that node and the corresponding 
        # neighbour distance to the source in the "result" list or by taking it directly from the
        # "result" list
        unvisited.remove(min_node)
        print("cur min: ", min_node)
...
def dijkstra_1(graph, source):
    ...
    # Consider all possible orders of traversing through the unvisited list/nodes, so need of tracking the 
    # smallest current distance in the result list
    length_loop = factorial(len(unvisited))
    i = 0
    while i < length_loop: 
...
print(dijkstra_1(testGraph, 'A'))     # {'A': 0, 'D': 2, 'B': 3, 'E': 3, 'C': 4}
...
print(dijkstra_1(graph, 'A'))        # {'A': 0, 'C': 10, 'B': 5}
...
print(dijkstra_1(graph, 'A'))       # {'A': 0, 'C': 3, 'B': 5, 'E': 4, 'D': 2, 'F': 6}
