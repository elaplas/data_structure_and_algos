...
def quick_sort(s_i, e_i, arr):
    
    if len(arr[s_i:e_i]) < 2:
        return
    
    pivot = e_i-1
    i = s_i
    while arr[i] != arr[pivot]:
        if arr[i] > arr[pivot] and i == pivot-1:
            tmp = arr[i]
            arr[i] = arr[pivot]
            arr[pivot] = tmp
            pivot -= 1
    ...
    quick_sort(s_i, pivot, arr)
    quick_sort(pivot, e_i, arr)
    
    
print(f"input: {items} ")        
quick_sort(0, len(items), items)
