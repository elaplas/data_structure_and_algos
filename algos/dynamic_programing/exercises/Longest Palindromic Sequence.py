...
def lps(input_string): 
    ...
    for i in range(len(input_string)-1, -1, -1):
        inv_str += input_string[i]
    
    num_cols = len(input_string)+1
    num_rows = len(inv_str)+1
    
    one_row = [0 for _ in range(num_cols)]
    matrix = [one_row for _ in range(num_rows)]
    
    for row in range(1, num_rows):
        for col in range(1, num_cols):
            
            if input_string[col-1] == inv_str[row-1]:
