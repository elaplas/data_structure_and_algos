...
def dijkstra(graph, start_node, end_node):
    result = dict.fromkeys([node.value for node in graph.nodes], float('inf'))
    result[end_node.value] = 0
    unvisited = set([node for node in graph.nodes])
    unvisited.remove(end_node)
    
    while len(unvisited):
        
        node_witn_min_dis_in_result = None
        for node in unvisited:
            
            if node_witn_min_dis_in_result is None:
                node_witn_min_dis_in_result = node
                
            
            cur_min_dis = result[node.value]
            for edge in node.edges:
                dis_from_neighbour = 0
                dis_from_neighbour += edge.distance
                dis_from_neighbour += result[edge.node.value]
                if dis_from_neighbour < cur_min_dis:
                    cur_min_dis = dis_from_neighbour
                    result[node.value] = cur_min_dis
                    
            
            if result[node.value] < result[node_witn_min_dis_in_result.value]:
                node_witn_min_dis_in_result = node
        
        unvisited.remove( node_witn_min_dis_in_result)
        
        if  node_witn_min_dis_in_result == start_node:
            break
            
    return result[start_node.value]
