...
class Queue:
    ...
    def __handel_full_capacitiy(self):
        if self.front_index == self.next_index and self.queue_size:
            index = self.front_index
            old_arr = self.arr
            self.arr = [0 for _ in range(len(old_arr) * 2)]
            i = 0
            while index <= self.next_index -1:
                self.arr[i] = old_arr[index]
                i += 1
                index = (index + 1) % len(old_arr)
