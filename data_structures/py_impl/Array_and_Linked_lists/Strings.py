...
def string_reverser(our_string):
    ...
    """
    our_string = list(our_string)
    # TODO: Write your solution here
    i = 0
    while i < len(our_string)/2:
        tmp = our_string[i]
        our_string[i] = our_string[len(our_string)-i-1]
        our_string[len(our_string)-1-i] = tmp
        i += 1
...
def word_flipper(our_string):
    ...
    for i in range(len(split_str)):
        split_str[i] = string_reverser(split_str[i])
